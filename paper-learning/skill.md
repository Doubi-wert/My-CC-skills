...
- **C (Critic)**: 批判性审查者，从批判角度审查生成内容是否符合参考文档

G 和 C 通过中间传递格式直接协作，无需独立协调者。

## 双层批判迭代架构

> **架构说明**：标题"双层"指**章节级 + 整体级**两层迭代组织方式。层级表中定义的Macro/Meso/Micro是迭代**处理粒度**的三个层次：
> - **Meso层**是章节级迭代的处理粒度
> - **Macro层**是整体级迭代的处理粒度
> - **Micro层**是Meso层的子组件，处理段落/局部级别的细节问题
>
> **layer与scope对应关系**：
> | layer层级 | 处理scope范围 | 说明 |
> |-----------|---------------|------|
> | Macro | global scope | 整体文档级别，全局一致性、前向引用、术语统一 |
> | Meso | chapter scope | 章节/模块级别，单章内容质量、章节内部逻辑 |
> | Micro | local scope | 段落/局部级别，局部表述、公式引用、细节准确 |
...
- **G**生成内容，通过中间传递格式将当前版本、反馈处理结果传递给C
- **C**审查内容，检查G的修订是否正确，记录反馈点，通过中间传递格式返回
- **收敛判定**由C执行，当反馈点为0或连续低阈值时触发退出
- **状态同步**通过中间传递格式自然完成，无需独立协调者
...
> **说明**：收敛判定相关的具体参数值请参考「配置参数」章节。本节仅描述收敛流程的高层判定逻辑。

### 迭代次数限制

- **章节级迭代**：单章最大迭代次数 = `max_chapter_iterations`（默认3次）
- **整体级迭代**：整体级最大迭代次数 = `max_global_iterations`（默认2次）
- **单轮反馈处理上限**：单轮最大处理反馈数 = 5个（超出部分自动deferred）
- **多章节总迭代估算**：总迭代次数 ≈ Σ(各章节迭代次数) + 整体迭代次数，单章迭代3次×章节数 + 整体迭代2次
...
当反馈点为0时:
1. **C审查完整性确认**: C需确认审查是否完整
   - 如果审查完整: 进入下一轮
   - 如果审查不完整: C要求重新审查，确认后方可进入下一轮
2. **下一轮仍为0**: 退出迭代
...
在无外部参照时，质量基准采用**自举基准 (Cootstrap Caseline)** 方法:
...
| Deferred超限 | 阈值型 | `consecutive_deferred_count >= max_consecutive_deferred` | >= 3次（即max_consecutive_deferred默认值） |
| 无效拒绝 | 阈值型 | `count(invalid_rejection) >= threshold` | >= 3次 |
| 质量下降 | 阈值型 | `quality_score_drop > 0.15` | 下降 > 0.15（与quality_threshold=0.1不同，此为触发人工介入的下降幅度阈值） |
| 配置冲突 | 规则型 | `static_config modified in iteration` | 即时触发 |
| 循环依赖 | 规则型 | `chapter_A waits chapter_C AND chapter_C waits chapter_A` | 即时触发 |
...
|---------|------|------|
| 重复反馈 | >= 3次 | 同一反馈点被重复提出 |
| Deferred超限 | >= 3次 | 同一问题连续deferred次数达到max_consecutive_deferred（consecutive_deferred_count >= max_consecutive_deferred） |
| 无效拒绝 | >= 3次 | G拒绝反馈但理由不充分 |
| 质量下降 | > 0.15 | 质量分数下降幅度超过0.15（注意：此阈值与quality_threshold=0.1不同） |
...
| `max_feedback_per_round` | 5 | 单轮最大处理反馈数（超出部分自动deferred到下一轮） |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `threshold` | 3 | 反馈点阈值（单轮反馈点超过此值则继续迭代） |
| `max_consecutive_deferred` | 3 | 连续deferred最大次数（连续deferred次数达到此值时触发强制升级） |
| `max_upgrade_retries` | 2 | 强制升级最大重试次数 |
| `convergence_rounds` | 2 | 连续低于阈值的轮数（连续此轮数反馈点<阈值时退出） |
| `quality_threshold` | 0.1 | 质量追踪模式的质量分数下降阈值（用于strict/relaxed模式的质量下降判定） |
